...
import { validatePhoneSignUp, getClientAddress, hasTrustedOrigin } from '@/lib/auth';
import { createAndDeliverOtpChallenge } from '@/lib/authSecurity';
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const validation = validatePhoneSignUp(body);
    if ('error' in validation) {
      return NextResponse.json({ error: validation.error }, { status: 400 });
    }

    const clientIp = getClientAddress(request);
    const limit = await verifyAuthRateLimit('register', validation.mobileNumber, clientIp);
    if (!limit.allowed) {
    ...
    console.error('Phone-based registration failed:', error instanceof Error ? error.message : 'Unknown error');
